...
> Compare communities using one NIST Tracking Community Resilience (TraCR)
> indicator over time with Python and reactive analytics.

## Live App

- [Live App](https://civic-interconnect.github.io/compare-tracr/)

## Compare Community Resilience Indicators
...
Select one resilience indicator and multiple communities to compare
how that measure changes over time.
...
- `s02_analytics.py` - produces comparison analytics
...
| `s01` | `s01_process_data.py`       | Basic cleaning: types, nulls, ordering |
| `s02` | `s02_analytics.py`          | `CompareResult` + `get_comparison()`   |
| `s03` | `s03_views.py`              | `CompareView` + `make_compare_view()`  |
| `s04` | `s04_charts_*.py`           | Renderer-specific chart creation       |

### Two contracts

`CompareResult` in `s02` is the boundary between analytics and visualization.
It carries observations for one indicator across multiple geographies:
...
Rankings, percent differences, latest-value comparisons, etc.
are separate analytics with their own small result types.

`CompareView` in `s03` is the boundary between view preparation and rendering.
It carries:
...
Each `s04_charts_*.py` exposes a renderer for the same `CompareView`:
...
- compare different TraCR indicators
- compare a different set of communities
- improve geography-selection controls
- add rankings or latest-year comparisons
- calculate differences between communities
...
```shell
git clone https://github.com/civic-interconnect/compare-tracr
cd compare-tracr
code .
```
